...
- **Jam Session resilience: 5-minute reconnect budget + WakeLock (2026-09-03)**: the Listen Together client previously gave up after five attempts in ~31s — short enough that a subway tunnel killed rooms. The reconnect loop now uses a 4-minute time budget matched to the server's `ReconnectGracePeriod` (5m), with `forceReconnect()` exposed for the network monitor to call on transport-type change. `JamWakeLockManager` is a base class with a no-op default; `AndroidJamWakeLockManager` (in `androidMain`) uses `PARTIAL_WAKE_LOCK` to keep the CPU alive while in a room.
  - **WebSocket ping is NOT the same as `MessageTypes.PING` (2026-09-03, issue #2347)**: a host trying to start a Jam room got "session not found or expired" / "you are not in a room" errors. The server logs showed `Room created` succeeded, then `User temporarily disconnected` fired **exactly 10.1–10.2 s** later, every time, across 7 room creation attempts. The Ktor install block was `install(WebSockets)` with no `pingIntervalMillis`, leaving Ktor's default `-1` (pings disabled) at the WebSocket transport layer. The `MessageTypes.PING`/`PONG` pair is an APPLICATION-level message that calibrates the clock — it does NOT keep the raw WebSocket connection alive. Render.com (where `metroserver-vjux.onrender.com` runs) sits behind a CDN/proxy with a ~10 s idle cutoff that severs the socket before the 15 s app-level ping fires. Fix: `install(WebSockets) { pingIntervalMillis = PING_INTERVAL.inWholeMilliseconds }` in `ListenTogetherClient.kt`. **Always set `pingIntervalMillis` whenever the server sits behind any reverse proxy — application-level pings do not protect the transport.** The session acquires the lock on `ROOM_CREATED` / `JOIN_APPROVED` and releases it on `leaveRoom`, `KICKED`, terminal `ERROR` codes, and a fatal `Disconnected` event. The DI shape follows the `cast`/`crashlytics` pattern: a `platformListenTogetherModule` (expect/actual) registered in `core/data` provides the right binding per platform; `core/data` already has `koin-android` in `androidMain`, so the Android actual simply calls `KoinJavaComponent.get<Application>()` from a lazy field.
  - **Why a base class, not `expect/actual` class.** `expect class Foo()` cannot be subclassed; an `actual class` must have the same constructor shape. With a base class instead, the JVM and iOS actuals are simply the no-op base, and only Android adds the override — same surface, no compiler gymnastics.
  - **PARTIAL_WAKE_LOCK, never anything stronger.** `FULL_WAKE_LOCK` is deprecated on Android 14+ and silently no-ops; `SCREEN_BRIGHT_WAKE_LOCK` would force the screen on, which the user does not want when their phone is in a pocket. The screen is allowed to sleep; only the CPU must stay alive for pings.
  - **Shutdown hook as a safety net.** A `Runtime.addShutdownHook` in the Android actual calls `release()` on JVM exit — the session machinery releases explicitly on every room-exit path, but a process that dies holding the lock would otherwise leak it. A leaked WakeLock prevents future rooms from acquiring it (system-wide), which is the worst case for this feature.
  - **Wake lock acquisition is paired with `ROOM_CREATED` / `JOIN_APPROVED`, NOT with the WebSocket handshake.** The socket connects before the user is in a room (so they can read settings, etc.), and the lock should only be held while a room is actually live. Releasing on every disconnect-with-no-retry, on KICKED, and on terminal error codes ensures the lock does not outlive the room.
  - **`forceReconnect()` is a separate method from `connect()`.** A transport handoff (Wi-Fi → cellular) needs to skip the backoff delay but should NOT reset the time budget — it is a continuation of the same room, not a new session. The network monitor calls `forceReconnect()`; the user-initiated path calls `connect()`.
  - **Time budget, not attempt count, governs the loop.** The first five attempts (1, 2, 4, 8, 16 s) match the old behaviour for transient blips. After that, the remaining budget is split into ~5s-spaced retries so the full 4-minute window is actually used. A `ReconnectState` `StateFlow` exposes `remainingMs` to the UI for a countdown indicator.
